...
inline void Player::cast_ray(ChunkManager& world, float FrameTime)
{
	...
	//Place Blocks
	if (Input::is_down(Key::Enter) && place_time > 0.2)
	{
		p -= Direction * 0.02;
	...
	//Break Blocks
	if (Input::is_down(Key::Backspace) && place_time > 0.2)
	{
		p += Direction * 0.02;
		int p_x = float_to_int(p.x);
		int p_y = float_to_int(p.y);
...
inline void Player::controls(float FrameTime)
{
	...
	//Block Selection
	if (Input::is_down(Key::Num1))
		block_selected = 1;
	if (Input::is_down(Key::Num2))
		block_selected = 2;
	if (Input::is_down(Key::Num3))
		block_selected = 3;
	if (Input::is_down(Key::Num4))
		block_selected = 4;
	if (Input::is_down(Key::Num5))
		block_selected = 5;
	if (Input::is_down(Key::Num6))
		block_selected = 6;
	if (Input::is_down(Key::Num7))
		block_selected = 7;
	...
	if (Input::is_down(Key::C))
		velocity.y = -5;
	if (Input::is_down(Key::F))
		is_flying = 1;
	if (Input::is_down(Key::X))
		is_flying = 0;
	if (Input::is_down(Key::Space) && (on_ground || is_flying))
	{
			velocity.y = 6;
			on_ground = 0;
	}
